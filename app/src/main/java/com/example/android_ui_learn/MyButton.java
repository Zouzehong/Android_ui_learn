...
public class MyButton extends androidx.appcompat.widget.AppCompatButton {
    private static final int INVALIDATE_DURATION = 25;     //每次刷新的时间间隔
    ...
    private void initPaint(){
        ...
        colorPaint.setColor(getResources().getColor(R.color.bottom_reveal));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_CANCEL:
                long time = SystemClock.elapsedRealtime() - downTime;
                if (time < TAP_TIMEOUT) {
                    DIFFUSE_GAP = 30;
                    postInvalidate();
                }else {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if(shaderRadio < maxRadio){
            postInvalidateDelayed(INVALIDATE_DURATION,pointX,pointY,pointX + viewWidth,
                    pointY + viewHeight);
